...
def find_optimal_candidate(candidates):
    top_score = 0
    optimal_candidate = None
    for idx in candidates:
        motion_val = foon_functional_units[idx].motion_node
        if motion_val in motions and motions[motion_val] > top_score:
            top_score = motions[motion_val]
            optimal_candidate = idx
...
def search_BFS(kitchen_items=[], goal_node=None):
    ...
    while len(items_to_search) > 0:
        ...
        if not check_if_exist_in_kitchen(kitchen_items, current_item):
            ...
            selected_candidate_idx = find_optimal_candidate(candidate_units)
        ...
        for object in foon_object_nodes:
            if object.check_object_equal(node_object):
                ...
                save_paths_to_file(output_task_tree,
                                   'output/output_heuristic1_{}.txt'.format(node["label"]))
                break
