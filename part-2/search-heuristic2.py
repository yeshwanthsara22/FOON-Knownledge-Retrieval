...
def total_ingredients(input_nodes):
    return sum(len(node.ingredients) for node in input_nodes)
    
def best_candidate(candidate_units):
    def heuristic(unit):
        inputs = len(unit.input_nodes)
        ingredients = total_ingredients(unit.input_nodes)
        return (inputs, ingredients)
    
    return min(candidate_units, key=lambda idx: heuristic(foon_functional_units[idx]))
...
def search_BFS(kitchen_items=[], goal_node=None):
    ...
    while len(items_to_search) > 0:
        ...
        if not check_if_exist_in_kitchen(kitchen_items, current_item):
            ...
            selected_candidate_idx = best_candidate(candidate_units)
...
if __name__ == '__main__':
    ...
    with open('motion.txt', 'r') as f:
        for line in f:
            line = line.rstrip()
            #spilt the line by tab
            line = line.split('\t')
            motions[line[0]] = float(line[1])
        ...
        for object in foon_object_nodes:
            if object.check_object_equal(node_object):
                ...
                save_paths_to_file(output_task_tree,
                                   'output/output_heuristic2_{}.txt'.format(node["label"]))
                break
